export default (document, currentPage) => {
  ...
  document.querySelectorAll('a').forEach(a => {
    ...
    if (canParse(a))
      ...
      url = new URL(a, 'https://helpx-internal.corp.adobe.com');

    if (!url.pathname.includes('/content/help/') || !findExtraHyphens.test(url.pathname)) return;
    const before = a.href;
    const after = url.pathname.replaceAll(findExtraHyphens, '-');
